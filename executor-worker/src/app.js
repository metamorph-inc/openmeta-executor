...
const app = (serverAddress, serverKey) => {
  ...
  const artifactDownload = async (artifactId, jobId) => {
    ...
    const writeStreamPromise = new Promise((resolve, reject) => {
      request.get(serverAddress + 'api/worker/downloadArtifact/' + artifactId)
  ...
  const uploadJobResults = async (job, resultCode) => {
    ...
    const uploadRes = await request.put(serverAddress + "api/worker/uploadArtifact")
      .set("openmeta-worker-key", serverKey)
      .attach("artifact", zipFilePath);

    console.log("Informing job server that job is complete");
    const res = await request.post(serverAddress + "api/worker/jobCompleted")
      .set("openmeta-worker-key", serverKey)
      .send({
        jobId: job.uid,
